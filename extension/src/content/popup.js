...
class LDNPopup {
    ...
    _init() {
        this._views = [];
        this._views[Constants.ViewState.IN_LOBBY] = document.getElementById('in-lobby-container');
        this._views[Constants.ViewState.OUT_LOBBY] = document.getElementById('out-lobby-container');
        this._views[Constants.ViewState.CONNECT_LOBBY] = document.getElementById('connect-lobby-container');
        this._viewState = new ViewState(this._views);
        this._controller = new PopupController(this._viewState);
        document.getElementById('start-lobby-btn').addEventListener('click', this._controller.dispatchStartLobby);
        document.getElementById('disconnect-btn').addEventListener('click', this._controller.dispatchConnectConfirm);
        document.getElementById('connect-btn').addEventListener('click', this._controller.dispatchConnect);
        document.getElementById('connect-btn-back').addEventListener('click', this._controller.back);
        document.getElementById('connect-confirm-btn').addEventListener('click', this._controller.dispatchConnect);
    }
