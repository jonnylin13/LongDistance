...
module.exports = class User extends JsonObject {
    ...
    static fromJson(jsonString) {
        ...
        if (!('lobbyId' in data) || !('id' in data) || !('controllerState' in data) || !('urlParams' in data) || !('progressState' in data)) {
            console.log('<Error> Tried to instantiate user with corrupt data!');
            return null;
        }
        return new User(data['lobbyId'], data['id'], data['controllerState'], data['urlParams'], ProgressState.fromJson(JSON.stringify(data['progressState'])));
    }
